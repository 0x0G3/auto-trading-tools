...
export default function GridBot({
  ...
  useEffect(() => {
    ...
    const fetchState = async () => {
      try {
        ...
        if (states && states.length > 0) {
          ...
          setIsRunning(runningSymbols.length > 0); // Sync with server state
    ...
    return () => {
      ...
      setIsPolling(false); // Reset polling flag only
    };
  ...
  const handleToggleBot = async (action: "start" | "stop") => {
    ...
    try {
      ...
      setIsRunning(action === "start"); // Update based on action, refine later if needed
    } catch (err) {
